...
const getTransactionPool = () => {
    return lodash.cloneDeep(transactionPool);
}
...
const getTxPoolIns = (aTransactionPool)=>{
    return lodash(aTransactionPool)
            .map((tx)=>tx.txIns)
            .flatten().value();
};
...
const isValidTxForPool = (tx, aTransactionPool)=>{
    ...
    const containsTxIn=(txIns, txIn)=>{
        return lodash.find(txPoolIns, ((txPoolIn)=>{
            return txIn.txOutId === txPoolIn.txOutId && txIn.txOutIndex === txPoolIn.txOutIndex;
        }))
    };
